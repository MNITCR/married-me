...
if (isset($_POST['submit_boy'])) {
    ...
    $checkDuplicateQuery = "SELECT girl_id FROM girl_tbl WHERE khmername = '$khname' AND englishname = '$egname' AND phone = '$phone' AND moneyriel = '$riel' AND moneydolar = '$dolar' AND locaid = (SELECT locaid FROM location_tbl WHERE village = '$village') AND user_id = (SELECT user_id FROM user_tbl WHERE user_id = '$userId')";
